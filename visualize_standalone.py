...
"""
=================================================================================
COMPREHENSIVE VISUALIZATION SCRIPT FOR SAC ACTIVE SUSPENSION
=================================================================================

This script generates ALL plots for active suspension analysis:
1. Time series comparisons (acceleration, displacement, force)
2. ISO 2631-5 comfort zones
3. Learning curves
4. Road class comparisons
5. Reward function comparisons
6. Metrics heatmaps
...
def plot_acceleration_comparison(time, active_accel, passive_accel, 
                                 ...
                                 save_path):
    """Plot acceleration: active vs passive"""
    fig, ax = plt.subplots(figsize=(14, 7))
    
    ax.plot(time, active_accel, 'b-', linewidth=2.5, label='Active (SAC)', alpha=0.9)
    ax.plot(time, passive_accel, 'r--', linewidth=2.0, label='Passive', alpha=0.8)
...
def plot_control_force(time, forces, u_max=8000, save_path='control_force.png'):
...
def plot_comfort_zones(active_rms, passive_rms, road_class, save_path):
    ...
    for i, (lower, upper, label, color) in enumerate(comfort_zones):
        ax.barh(i, upper - lower, left=lower, height=0.8,
               color=color, alpha=0.6, label=label, edgecolor='black', linewidth=1)
    
    # Plot RMS values
    for i, (lower, upper, _, _) in enumerate(comfort_zones):
        if lower <= active_rms < upper:
            ax.plot(active_rms, i, 'b*', markersize=25, label='Active SAC', zorder=10)
        if lower <= passive_rms < upper:
            ax.plot(passive_rms, i, 'ro', markersize=20, label='Passive', zorder=10)
...
def plot_learning_curves(timesteps, rewards, rms_values, save_path):
    ...
    # Rewards
    ax1.plot(timesteps, rewards, 'b-', linewidth=2.0, alpha=0.7, label='Episode Reward')
    ...
    # RMS values
    if rms_values:
        ax2.plot(timesteps, rms_values, 'g-', linewidth=2.0, alpha=0.7, label='RMS Acceleration')
...
def plot_reward_comparison(reward_ids, rms_values, descriptions, save_path):
    """Compare different reward functions"""
    fig, ax = plt.subplots(figsize=(14, 8))
    
    # Sort by performance
    sorted_indices = np.argsort(rms_values)
    reward_ids = [reward_ids[i] for i in sorted_indices]
    rms_values = [rms_values[i] for i in sorted_indices]
    descriptions = [descriptions[i] for i in sorted_indices]
    
    colors = ['#2ecc71' if rms < 0.228 else '#e74c3c' for rms in rms_values]
    
    y_pos = np.arange(len(reward_ids))
    bars = ax.barh(y_pos, rms_values, color=colors, alpha=0.8, edgecolor='black')
    
    ax.set_yticks(y_pos)
    ax.set_yticklabels([f"R{rid}: {desc}" for rid, desc in zip(reward_ids, descriptions)],
                      fontsize=10)
    ax.set_xlabel('RMS Acceleration (m/s²)', fontsize=14, fontweight='bold')
    ...
    ax.axvline(x=0.228, color='blue', linestyle='--',
              linewidth=2.5, label='Paper Target (0.228)', alpha=0.8)
    
    for i, (bar, rms) in enumerate(zip(bars, rms_values)):
        ax.text(rms + 0.01, i, f'{rms:.4f}', 
...
def visualize_all(results_dir, output_dir):
    ...
    if comparison_file.exists():
        ...
        dt = 0.01
        time = np.arange(len(time_series['active']['accelerations'])) * dt
        ...
        plot_control_force(
            ...
            save_path=output_dir / 'control_force.png'
        )
        
        # Plot 3: Comfort zones
        plot_comfort_zones(
            active_metrics['rms_acceleration'],
            passive_metrics['rms_acceleration'],
            road_class,
...
def main():
    parser = argparse.ArgumentParser(description='Visualize SAC results')
